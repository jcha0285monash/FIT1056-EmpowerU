...
class StaffTeacherMenu(tk.Frame):
    def __init__(self, master, staff_user, staff_menu):
        ...
        self.edit_teacher_button.grid(row=3, column=0, padx=10, pady=10)

        # add teacher button
        self.add_teacher_button = tk.Button(self, text="Add Teacher", command=self.add_teacher_button_clicked)
        self.add_teacher_button.grid(row=3, column=1, padx=10, pady=10)
        ...
        self.delete_teacher_button.grid(row=3, column=2, padx=10, pady=10)
    ...
    def edit_teacher_button_clicked(self):
        ...
        # teacher course text widget entry
        self.teacher_course_edit_entry = tk.Text(teacher_edit_window, width=50, height=5)
        self.teacher_course_edit_entry.insert(tk.END, self.selected_teacher_course)
    ...
    def save_button_clicked(self):
        ...
        new_course = self.teacher_course_edit_entry.get("1.0", tk.END).strip().split("\n")
        ...
        if uid == "" or name == "" or email == "" or password == "":
            ...
            return
        teacher_details = uid + "," + email + "," + password + "," + name + "," + course + "," + self.selected_teacher_status
    ...
    def activate_deactivate_teacher_button_clicked(self):
        ...
        children = self.tree.get_children(parent)
        self.selected_line = children.index(selected_data)
        teacher_courses = teacher_details[4].replace("\n", "&")
        if teacher_details[5] == "DEACTIVATED":
            if os.path.exists(self.teacher_path):
                with open(self.teacher_path, "r", encoding="utf-8") as rf:
                    data = rf.readlines()
                data[self.selected_line] = teacher_details[0] + "," + teacher_details[1] + "," + teacher_details[2] + "," + teacher_details[3] + "," + teacher_courses + ",ACTIVE\n"
                with open(self.teacher_path, "w", encoding="utf-8") as wf:
                    wf.writelines(data)
                self.load_teachers()
        ...
        if os.path.exists(self.teacher_path):
            with open(self.teacher_path, "r", encoding="utf-8") as rf:
                data = rf.readlines()
            data[self.selected_line] = teacher_details[0] + "," + teacher_details[1] + "," + teacher_details[2] + "," + teacher_details[3] + "," + teacher_courses + ",DEACTIVATED\n"
            with open(self.teacher_path, "w", encoding="utf-8") as wf:
                wf.writelines(data)
            self.load_teachers()
            self.alert_label.config(fg="green")
